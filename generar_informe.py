...
const LOGO_PATH = process.env.LOGO_PATH || "/app/logo_transparente.png";
const {
...
function calcDesb(r,s,t){
  const vals=[r,s,t].map(v=>parseFloat(v)).filter(v=>!isNaN(v)&&v>0);
  if(vals.length<3)return null;
  const avg=vals.reduce((a,b)=>a+b,0)/vals.length;
  if(avg===0)return null;
  return(Math.max(...vals.map(v=>Math.abs(v-avg)))/avg)*100;
}
function alertaTipo(tipo,val){
  const v=parseFloat(val);if(isNaN(v)||val===null||val===undefined)return null;
  if(tipo==='tri'){if(v>=368&&v<=391)return null;if((v>=361&&v<=367)||(v>=392&&v<=399))return'warn';return'crit';}
  if(tipo==='mono'){if(v>=213&&v<=227)return null;if((v>=209&&v<=212)||(v>=228&&v<=231))return'warn';return'crit';}
  if(tipo==='ntie'){if(v>=0&&v<=2)return null;if(v>2&&v<=5)return'warn';return'crit';}
  if(tipo==='temp'){if(v<=40)return null;if(v<=50)return'warn';return'crit';}
  if(tipo==='desb'){if(v<=3)return null;if(v<=5)return'warn';return'crit';}
  return null;
}
function alertasTablero(tb){
  const probs=[];
  const chk=(label,tipo,val)=>{const s=alertaTipo(tipo,val);if(s)probs.push({p:label,v:String(val??'—'),s,c:s==='crit'?'C00000':'7F6000'});};
  chk('V R/S','tri',tb.v_rs);chk('V R/T','tri',tb.v_rt);chk('V S/T','tri',tb.v_st);
  chk('V R/N','mono',tb.v_rn);chk('V S/N','mono',tb.v_sn);chk('V T/N','mono',tb.v_tn);
  chk('V R/Tie','ntie',tb.v_rtie);chk('V S/Tie','ntie',tb.v_stie);chk('V T/Tie','ntie',tb.v_ttie);chk('V N/Tie','ntie',tb.v_ntie);
  chk('T° R','temp',tb.t_r);chk('T° S','temp',tb.t_s);chk('T° T','temp',tb.t_t);chk('T° N','temp',tb.t_n);chk('T° Tie','temp',tb.t_tie);
  const desb=tb.desbalance??calcDesb(tb.i_r,tb.i_s,tb.i_t);
  if(desb!==null&&!isNaN(desb)){const s=alertaTipo('desb',desb);if(s)probs.push({p:'Desbalance I',v:parseFloat(desb).toFixed(1)+'%',s,c:s==='crit'?'C00000':'7F6000'});}
  return probs;
}

// ── Sección resumen de alertas ─────────────────────────────────────────────
function seccionAlertas(tableros){
  const W=9360;
  const items=[];
  items.push(new Paragraph({spacing:{before:240,after:100},children:[txt('Resumen de alertas',{bold:true,size:28,color:COLOR_AZUL})]}));

  const conAlertas=tableros.map(tb=>({tb,probs:alertasTablero(tb)})).filter(x=>x.probs.length>0);

  if(!conAlertas.length){
    items.push(par([txt('✓  Todos los parámetros se encuentran dentro de los rangos normales.',{size:20,color:'059669',bold:true})],{after:200}));
    return items;
  }

  const criticos=conAlertas.filter(x=>x.probs.some(p=>p.s==='crit')).length;
  const advertencias=conAlertas.filter(x=>x.probs.every(p=>p.s==='warn')).length;
  items.push(par([
    txt(`Se detectaron alertas en ${conAlertas.length} tablero(s): `,{size:20}),
    txt(`${criticos} CRÍTICO(S)`,{size:20,bold:true,color:'C00000'}),
    txt(`  |  `,{size:20,color:'888888'}),
    txt(`${advertencias} ADVERTENCIA(S)`,{size:20,bold:true,color:'7F6000'}),
  ],{after:160}));

  const hdr=new TableRow({children:[
    celdaH('N°',480,COLOR_AZUL),celdaH('Tablero',2800,COLOR_AZUL),celdaH('Piso',480,COLOR_AZUL),
    celdaH('Parámetro',1600,COLOR_AZUL),celdaH('Valor',720,COLOR_AZUL),
    celdaH('Estado',1480,COLOR_AZUL),celdaH('Hora',1300,COLOR_AZUL),
  ]});
  const rows=[hdr];
  conAlertas.forEach(({tb,probs})=>{
    probs.forEach((prob,idx)=>{
      const isCrit=prob.s==='crit';
      const rf=isCrit?'FFF0F0':'FFFBEB';
      rows.push(new TableRow({children:[
        celda(idx===0?String(tb.n):'',480,{fill:rf}),
        celda(idx===0?tb.nombre:'',2800,{fill:rf,bold:idx===0,align:AlignmentType.LEFT}),
        celda(idx===0?String(tb.piso):'',480,{fill:rf}),
        celda(prob.p,1600,{fill:rf,align:AlignmentType.LEFT}),
        celda(prob.v,720,{fill:rf,bold:true}),
        celda(isCrit?'⚠ CRÍTICO':'⚡ ADVERTENCIA',1480,{fill:isCrit?'FFE4E4':'FFF3CD',bold:true,color:prob.c}),
        celda(tb.fecha_hora?(tb.fecha_hora.split(' ')[1]||'—'):'—',1300,{size:15}),
      ]}));
    });
  });
  items.push(new Table({width:{size:W,type:WidthType.DXA},columnWidths:[480,2800,480,1600,720,1480,1300],rows}));
  items.push(par([txt('')],{after:200}));
...
async function buildDoc(datos, grafPaths) {
  ...
  // Resumen de alertas
  const alertasSection = seccionAlertas(tbs);
  ...
  resumen.push(new Table({width:{size:9360,type:WidthType.DXA},columnWidths:[4680,4680],rows:[
    new TableRow({children:[
      new TableCell({width:{size:4680,type:WidthType.DXA},borders:sinBordes(),margins:{top:100,bottom:100,left:120,right:120},children:[
        par([txt("Elaborado por:",{bold:true,size:18,color:COLOR_AZUL})],{after:datos.firma_b64?80:400}),
        ...(datos.firma_b64?[new Paragraph({alignment:AlignmentType.LEFT,spacing:{before:0,after:80},children:[imgRun(Buffer.from(datos.firma_b64,'base64'),5,1.5)]})]:[par([txt("_______________________________",{size:18,color:"888888"})],{after:60})]),
  ...
  const doc = new Document({
    ...
    sections:[{
      ...
      children:[...portada,...alertasSection,...intro,...parametros,...grafSection,...fotosSection,...resumen]
    }]
  });
  return doc;
}
