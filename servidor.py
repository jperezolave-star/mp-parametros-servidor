...
def instalar_y_encontrar_node():
    ...
    # Buscar con which (excluyendo /proc)
    try:
        r = subprocess.run(["which", "node"], capture_output=True, timeout=5)
        if r.returncode == 0:
            path = r.stdout.decode().strip()
            if "/proc" not in path and os.path.isfile(path):
                print(f"✓ Node en PATH: {path}")
                return path
    except: pass
    ...
    try:
        ...
        if r.returncode == 0:
            ...
            if "/proc" not in path:
                ...
                subprocess.run(["npm", "install", "--prefix", "/app",
                                "docx", "openpyxl"],
                               capture_output=True, timeout=120)
                print(f"✓ Node instalado: {path}")
                return path
    except Exception as e:
        print(f"Error instalando Node: {e}")

    print("✗ Node.js no disponible")
    return None

NODE_PATH = instalar_y_encontrar_node()
print(f"Node final: {NODE_PATH}")
...
def health():
    return jsonify({"status":"ok","app":"MP Ingeniería — Servidor de parámetros",
                    "version":"1.2.0","node":NODE_PATH or "no encontrado"})
    ...
    try:
        ...
        if NODE_PATH:
            ...
            if not os.path.exists(node_modules):
                subprocess.run(["npm", "install", "--prefix", "/app", "docx"],
        ...
        return jsonify({"ok":True,
                        ...
                        "word_url":word_url or "Word no disponible — Node.js no encontrado"})
